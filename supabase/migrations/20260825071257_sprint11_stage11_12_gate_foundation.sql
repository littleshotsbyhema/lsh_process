...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: mark_booking_selection_pending already exists';
  END IF;
  ...
  IF to_regclass('public.bookings') IS NULL
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: required canonical relation missing';
  END IF;
  ...
  IF to_regprocedure(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: required canonical helper unavailable';
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: canonical booking.stage.advance permission unavailable';
  END IF;
  ...
  IF v_count <> 3 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: canonical booking.stage.advance grant cardinality changed';
  END IF;
  ...
  IF v_count <> 3 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: canonical booking.stage.advance role boundary unavailable';
  END IF;
  ...
  IF v_count <> 2 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 precondition failed: canonical Stage 11 / Stage 12 catalogue unavailable';
  END IF;
END
...
BEGIN
  IF to_regprocedure(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: Stage 11 -> 12 RPC unavailable';
  END IF;
  ...
  IF v_count <> 1 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: Stage 11 -> 12 RPC signature/security contract invalid';
  END IF;
  ...
  IF NOT has_function_privilege(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: authenticated EXECUTE unavailable';
  END IF;
  ...
  IF has_function_privilege(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: anon EXECUTE must be denied';
  END IF;
  ...
  IF has_function_privilege(
    ...
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: service_role EXECUTE must remain unavailable';
  END IF;
  ...
  IF v_count <> 3 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: booking.stage.advance grant cardinality changed unexpectedly';
  END IF;
  ...
  IF v_count <> 3 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: booking.stage.advance role boundary changed unexpectedly';
  END IF;
  ...
  IF v_count <> 0 THEN
    RAISE EXCEPTION
      'Sprint 11 Slice 4 validation failed: unexpected stage-specific selection permission exists';
  END IF;
END
