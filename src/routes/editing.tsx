    ...
    <AppShell>
      ...
      {editing.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">Start one from a booking once selection + payment are confirmed.</p>
        </Card>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted text-muted-foreground text-[11px] uppercase tracking-wider">
              <tr>
                ...
                <Th>Selection rcvd</Th><Th>Deadline</Th><Th>QC</Th><Th>Delivery</Th><Th></Th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {editing.map((e) => (
                <tr key={e.id} className="hover:bg-muted/40">
                  <Td><span className="font-medium text-primary">{e.id}</span><div className="text-[10px] text-muted-foreground">{e.bookingId}</div></Td>
                  <Td>{e.client}</Td>
