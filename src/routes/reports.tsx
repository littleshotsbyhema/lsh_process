...
import { useStore, bookingFlags, bookingAlignment } from "@/store/useStore";
...
function ReportsPage() {
  ...
  const revenue = bookings.reduce((sum, b) => sum + (b.payment === "Paid" ? b.offer : b.advance), 0);
  ...
  bookings.forEach((b) => { packageCounts[b.package] = (packageCounts[b.package] ?? 0) + 1; });
  ...
  const editingDelays = editing.filter((e) => e.deadline !== "—" && e.deadline < todayDate && e.status !== "Delivered").length;
  const heirloomDelays = heirloom.filter((h) => !h.delivered).length;
  const reviewsPending = reviews.filter((r) => r.requestStatus === "Pending" || r.requestStatus === "Requested").length;
  ...
  const marketingPending = tasks.filter((t) => t.role === "Marketing Team" && t.status === "Pending").length;

  const cards = [
    { label: "Monthly revenue (collected)", value: `₹${revenue.toLocaleString("en-IN")}` },
    { label: "Booking conversion", value: `${conversion}%` },
    { label: "Pending privacy issues", value: pendingPrivacy, tone: pendingPrivacy ? "bad" : "good" },
    { label: "Safety checklist completion", value: `${safetyPct}%`, tone: safetyPct >= 95 ? "good" : "warn" },
    { label: "Editing delays", value: editingDelays, tone: editingDelays ? "warn" : "good" },
    { label: "Album / frame delays", value: heirloomDelays, tone: heirloomDelays ? "warn" : "good" },
    { label: "Review requests pending", value: reviewsPending, tone: reviewsPending ? "warn" : "good" },
    ...
    { label: "Philosophy Alignment Score", value: alignmentAvg ? `${alignmentAvg}/5` : "—", tone: "gold" },
    { label: "Marketing content pending approval", value: marketingPending, tone: marketingPending ? "warn" : "good" },
  ] as const;
      ...
      <section className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-4 mb-8">
        {cards.map((c) => (
          <Card key={c.label} className="p-5">
            ...
            {"tone" in c && c.tone && <div className="mt-2"><StatusPill tone={c.tone as "good" | "warn" | "bad" | "gold"}>{String(c.value)}</StatusPill></div>}
          </Card>
        ))}
      </section>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <div className="mt-5 border-t border-border pt-4">
            <h3 className="text-xs uppercase tracking-wider text-muted-foreground mb-2">Top packages</h3>
            <ul className="space-y-1.5">
              {Object.entries(packageCounts).sort((a, b) => b[1] - a[1]).slice(0, 4).map(([k, v]) => (
                <li key={k} className="flex items-center justify-between text-sm">
