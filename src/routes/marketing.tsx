...
function MarketingPage() {
  ...
  const blocked = bookings.filter((b) => !bookingFlags(b).marketingAllowed && bookingFlags(b).consentRecorded === false);
    ...
    <AppShell>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-8">
        <Card className="p-6">
          ...
          {approved.length === 0 ? (
            <p className="text-sm italic text-muted-foreground">Every shared memory is protected with consent.</p>
          ) : (
        ...
        <Card className="p-6">
          ...
          {blocked.length === 0 ? (
            <p className="text-sm italic text-muted-foreground">All bookings have a consent decision recorded.</p>
          ) : (
            <ul className="divide-y divide-border">
              {blocked.map((b) => (
                <li key={b.id} className="py-3 flex items-center justify-between">
                  <div>
                    <div className="font-medium text-primary">{b.client}</div>
                    <div className="text-xs text-muted-foreground">{b.id} · {b.category}</div>
                  </div>
                  <Link to="/privacy" className="text-xs px-3 py-1.5 rounded-lg border border-border bg-card hover:bg-accent">Record consent</Link>
                </li>
              ))}
            </ul>
          )}
            ...
            {marketingTasks.map((t) => (
              <li key={t.id} className="py-3 flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-muted-foreground">{t.relatedLabel ?? "—"} · due {t.dueDate}</div>
                </div>
                <div className="flex items-center gap-2">
                  <StatusPill tone={t.status === "Done" ? "good" : t.status === "Blocked" ? "bad" : "warn"}>{t.status}</StatusPill>
                  {t.status !== "Done" && (
                    <button
                      onClick={() => { const r = updateTask(t.id, { status: "Done" }); if (r.ok) toast.success(r.message); }}
