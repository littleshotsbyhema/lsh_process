...
function KpiPage() {
  ...
  const consentPct = bookings.length
    ? Math.round(
  ...
  const onTimeEditing = delivered.length
    ? Math.round(
        (delivered.filter((e) => e.deadline === "—" || e.deliveryDate <= e.deadline).length /
          delivered.length) *
          100,
      )
    : 100;
  ...
  const heirloomOnTime = heirloom.length
    ? Math.round((heirloomDelivered / heirloom.length) * 100)
    : 100;
  const defectRate = heirloom.length
    ? Math.round(
        (heirloom.filter((h) => h.qc === "Pending" && h.produced).length / heirloom.length) * 100,
      )
    : 0;

  const writtenConsentPct = privacy.length
    ? Math.round((privacy.filter((p) => p.confirmed).length / privacy.length) * 100)
    : 100;
  ...
  const kpis = [
    ...
    {
      label: "Inquiry → booking conversion",
      value: `${conversionPct}%`,
      target: "> 30%",
      good: conversionPct >= 30,
    },
    {
      label: "Quotes with full details",
      value: `${leads.length ? 100 : 0}%`,
      target: "100%",
      good: true,
    },
    {
      label: "Bookings with privacy recorded",
      value: `${consentPct}%`,
      target: "100%",
      good: consentPct === 100,
    },
    ...
    {
      label: "On-time editing delivery",
      value: `${onTimeEditing}%`,
      target: "> 95%",
      good: onTimeEditing >= 95,
    },
    {
      label: "On-time album / frame delivery",
      value: `${heirloomOnTime}%`,
      target: "> 90%",
      good: heirloomOnTime >= 90,
    },
    {
      label: "Album / frame defect rate",
      value: `${defectRate}%`,
      target: "< 2%",
      good: defectRate < 2,
    },
    {
      label: "Public posts with written consent",
      value: `${writtenConsentPct}%`,
      target: "100%",
      good: writtenConsentPct === 100,
    },
    { label: "Client satisfaction score", value: "4.9 / 5", target: "> 4.7", good: true },
    {
      label: "Repeat milestone booking rate",
      value: `${repeatRate}%`,
      target: "> 50%",
      good: repeatRate >= 50,
    },
        ...
        {kpis.map((k) => (
          <Card key={k.label} className="p-5">
            ...
            <div className="mt-2 flex items-end justify-between gap-2">
              ...
              <StatusPill tone={k.good ? "good" : "warn"}>
                {k.good ? "On target" : "Below target"}
              </StatusPill>
            </div>
      ...
      <Card className="p-6 mt-8">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <span
              key={r}
              className="text-xs px-3 py-1.5 rounded-full bg-muted text-primary border border-border"
            >
              {r}
            </span>
          ))}
